...
Trong quá trình tham gia Bootcamp, em cùng các thành viên trong nhóm thực hiện dự án **FoodieRecipe** — ứng dụng chia sẻ công thức nấu ăn có tích hợp AI. Sản phẩm cho phép người dùng quản lý và tìm kiếm công thức, tải ảnh món ăn lên hệ thống, nhận diện nội dung ảnh và nhận các gợi ý hỗ trợ tạo công thức. Kiến trúc tổng thể sử dụng **Next.js** trong thư mục `web` cho Frontend, **NestJS** trong thư mục `api` cho Backend, **Amazon RDS for PostgreSQL** để lưu dữ liệu quan hệ, cùng các dịch vụ AWS phục vụ lưu trữ, xử lý AI, phân phối nội dung, bảo mật và giám sát.

Phần công việc chính của em tập trung vào luồng xử lý ảnh thông minh. Em tìm hiểu và xây dựng cơ chế để Backend cấp **S3 pre-signed URL**, cho phép trình duyệt tải ảnh trực tiếp lên **Amazon S3** mà không truyền toàn bộ tệp qua API. Sau khi upload hoàn tất, **Amazon Rekognition** được sử dụng để nhận diện nhãn và kiểm duyệt nội dung ảnh. Những ảnh hợp lệ cùng kết quả nhận diện được chuyển đến mô hình thông qua **Amazon Bedrock** để gợi ý tên món ăn, mô tả, nguyên liệu và tag. Người dùng có thể xem lại, chỉnh sửa và xác nhận nội dung do AI đề xuất trước khi lưu công thức.
...
Bên cạnh đó, em còn tham gia phân tích yêu cầu, xây dựng workflow, thiết kế kiến trúc, viết tài liệu Proposal và Workshop song ngữ, kiểm thử các bước của luồng xử lý ảnh và hoàn thiện báo cáo theo góp ý. Kiến trúc sản phẩm hoàn chỉnh có sử dụng **Amazon EC2, Docker, Nginx, Amazon RDS, AWS Secrets Manager, IAM và Amazon CloudWatch** để vận hành Backend, quản lý dữ liệu, bảo vệ thông tin nhạy cảm và theo dõi hệ thống. Tuy nhiên, em không trực tiếp phụ trách việc triển khai Backend lên EC2; phần đóng góp của em tập trung vào tích hợp Next.js, NestJS và luồng ảnh sử dụng S3, Rekognition, Bedrock và CloudFront.
...
| --- | --- | --- | :---: | :---: | :---: |
| 1 | **Kiến thức và kỹ năng chuyên môn** | Áp dụng kiến thức AWS Cloud, Next.js, NestJS và AI vào luồng xử lý ảnh của FoodieRecipe | ☐ | ✅ | ☐ |
...
| 11 | **Đóng góp cho dự án** | Hoàn thành luồng ảnh AI và đóng góp Proposal, Workshop cùng tài liệu dự án | ☐ | ✅ | ☐ |
